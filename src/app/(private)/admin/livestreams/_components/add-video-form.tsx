...
const AddVideoTriggerButton = (
  <div className="rounded-lg overflow-hidden bg-white  shadow-sm border-2 border-dashed border-gray-300 hover:border-gray-300 hover:border-gray-400 transition-colors bg-gray-50 hover:bg-gray-100 transition-all">
    <div className="aspect-video w-full  flex flex-col items-center justify-center ">
      ...
      <span className="mt-2 text-sm text-gray-500">Add New Video</span>
    </div>
...
export function AddVideoForm({ onSuccess }: AddVideoFormProps) {
  ...
  if (isDesktop) {
    return (
      <Dialog>
        <DialogTrigger asChild>{AddVideoTriggerButton}</DialogTrigger>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            <DialogTitle>Add New Video</DialogTitle>
            <DialogDescription>
              Add a new video from YouTube or Facebook
            </DialogDescription>
          </DialogHeader>
  ...
  return (
    <Drawer>
      <DrawerTrigger asChild>{AddVideoTriggerButton}</DrawerTrigger>
      <DrawerContent>
        <DrawerHeader>
